...
function default_1(tree) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        devkit_1.updateJson(tree, 'package.json', (json) => {
            ...
            json.devDependencies = Object.assign(Object.assign({}, json.devDependencies), { '@angular/animations': '^12.0.0', '@angular/common': '^12.0.0', '@angular/compiler': '^12.0.0', '@angular/compiler-cli': '^12.0.0', '@angular/core': '^12.0.0', '@angular/forms': '^12.0.0', '@angular/platform-browser': '^12.0.0', '@angular/platform-browser-dynamic': '^12.0.0', '@angular/router': '^12.0.0', '@nativescript/angular': '^12.0.0', '@nativescript/core': '^8.0.0', '@nativescript/webpack': 'beta', '@nativescript/types': '^8.0.0', '@ngtools/webpack': '^12.0.0', husky: '^5.1.3', 'nativescript-vue': '~2.9.0', 'nativescript-vue-template-compiler': '~2.9.0', 'ng-packagr': '^12.0.0', 'zone.js': '~0.11.1', typescript: '~4.2.0' });
            delete json['husky'];
            json['lint-staged'] = {
...
function updateDemoFiles(tree, type) {
    const demoPath = `apps/demo${type ? '-' + type : ''}`;
    console.log(`Updating webpack config for: ${demoPath}`);
    const cwd = process.cwd();
    const webpackConfigPath = path_1.join(cwd, `node_modules/@nativescript/plugin-tools/src/migrations/update-2-0-0/_files_demo${type ? '_' + type : ''}/webpack.config.js`);
    ...
    const appCssPath = path_1.join(cwd, demoAppCssPath);
    // console.log('webpackConfigPath:', webpackConfigPath);
  ...
  @import 'nativescript-theme-core/scss/index';`;
    ...
    switch (type) {
        case 'angular':
            // add polyfills
            const polyfillsPath = path_1.join(cwd, `node_modules/@nativescript/plugin-tools/src/migrations/update-2-0-0/_files_demo_angular/polyfills.ts__tmpl__`);
...
runNativeScriptAngularApp({
  appModuleBootstrap: () => platformNativeScript().bootstrapModule(AppModule),
        ...
        `;
            ...
            devkit_1.updateJson(tree, tsconfigPath, (json) => {
                if (json && json.files) {
...
function updatePackages(tree) {
    ...
    for (const dir of packageFolders) {
        ...
        if (tree.exists(ngPackagePath)) {
            devkit_1.updateJson(tree, ngPackagePath, (ngPackageJson) => {
            ...
            devkit_1.writeJson(tree, `${packageDir}/angular/tsconfig.json`, tsconfgNg);
        }
    }
}
function updateTSConfig(tree) {
    const baseTsConfigPath = 'tsconfig.base.json';
    devkit_1.updateJson(tree, baseTsConfigPath, (tsconfigJson) => {
...
function addHuskyPrecommit(tree) {
    console.log(`Updating husky precommit...`);
    devkit_1.generateFiles(tree, devkit_1.joinPathFragments(__dirname, '_files_husky'), '', {
        tmpl: '',
        dot: '.',
    });
}
