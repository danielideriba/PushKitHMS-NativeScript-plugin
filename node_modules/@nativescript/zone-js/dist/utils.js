...
export function patchNativeScriptEventTarget(global, api, apis, patchOptions) {
    ...
    function patchNativeScriptEventTargetMethods(obj, patchOptions) {
        ...
        //   return false;
        // }
        function compare(task, delegate, thisArg) {
        ...
        const nativeAddListener = api.patchMethod(obj, ADD_EVENT_LISTENER, (delegate, delegateName, name) => function (originalTarget, originalArgs) {
            const addSingleEvent = function (target, args) {
                ...
                const objName = obj.name || ((_a = obj === null || obj === void 0 ? void 0 : obj.constructor) === null || _a === void 0 ? void 0 : _a.name);
                const task = Zone.current.scheduleEventTask(objName + ':' + (eventNameToString ? eventNameToString(eventName) : eventName), callback, data, schedule, unschedule);
            ...
            const events = typeof originalArgs[0] === 'string' ? originalArgs[0].split(',') : [];
            if (events.length > 0) {
        ...
        const nativeOnce = api.patchMethod(obj, ONCE, (delegate, delegateName, name) => function (originalTarget, originalArgs) {
            const addSingleEvent = function (target, args) {
                var _a;
                const eventName = args[0];
                const callback = args[1];
            ...
            const events = originalArgs && Array.isArray(originalArgs) && typeof originalArgs[0] === 'string' ? originalArgs[0].split(',') : [];
            if (events.length > 0) {
        ...
        const nativeRemoveListener = api.patchMethod(obj, REMOVE_EVENT_LISTENER, (delegate, delegateName, name) => function (originalTarget, originalArgs) {
...
export function patchClass(className, api) {
    ...
    _global[className] = function () {
        const a = api.bindArguments(arguments, className);
        switch (a.length) {
            case 0:
