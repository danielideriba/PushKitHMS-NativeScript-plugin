...
function initLocal(workspace) {
    ...
    //nx-ignore-next-line
    const supportedNxCommands = require('@nrwl/workspace/src/command-line/supported-nx-commands').supportedNxCommands;
    ...
    if (supportedNxCommands.includes(process.argv[2])) {
        ...
        if (workspace.type === 'nx') {
            ...
            if (process.argv[2] === 'update' &&
                ...
                const pm = package_manager_1.getPackageManagerCommand();
...
function runOneOptions(workspace) {
    try {
        const workspaceConfigJson = workspace_1.resolveNewFormatWithInlineProjects(fileutils_1.readJsonFile(path.join(workspace.dir, workspace.type === 'nx' ? 'workspace.json' : 'angular.json')));
        const nxJson = fileutils_1.readJsonFile(path.join(workspace.dir, 'nx.json'));
        return parse_run_one_options_1.parseRunOneOptions(workspace.dir, workspaceConfigJson, nxJson, process.argv.slice(2));
    }
    catch (_a) {
        return false;
    }
